...
# 번역 사전 의미
# - REST API는 국제표준을 따라 영어 필드명 사용
# - 학습 데이터와 인코드는 데이터 분석가/데이터 기획자가 다루기 쉬운 한글 컬럼명으로 만들어졌다
# - 두 상황을 잇는 번역 사전 역할 → FIELD_TO_COLUMN
FIELD_TO_COLUMN = {
    "age": "나이",
    "gender": "성별",
    "annual_income": "연소득",
...
class LoanModel:
    ...
    (이 클래스가 pipeline, label_encoders, feature_names까지 로드하는 이유,
    학습 시 저장했던 전처리 도구를 그대로 재사용해야 두 단계의 결과가 일치한다.)
    """
    def __init__(self):
        # 생성 직후에는 아직 적재되지 않은 상태
        # main.py에서 /health (헬스체크) 할 때 self.pipeline = None이면 초기 상태
        self.pipeline = None
        ...
        # ex) 재학습된 새 모델을 배포했을 때 예측 로그에 버전을 함께 남기면
        #     어느 버전의 모델이 이 예측을 했는지 추적할 수 있어 드리프트 분석이나 문제 발생 시 원인 추적에 도움이 된다.

    def load(self, model_dir: str = 'models') -> None:
        """
        모델, 레이블 인코더, 피처 특성(컬럼) 순서 파일을 디스크에서 불러온다.

        현재 작업 폴더가 어디든 동일하게 동작하도록 이 파일의 위치를 기준으로 계산
        Dockerfile의 WORKDIR이 달라져도 이 코드는 항상 이 파일(model.py) 기준으로 models 폴더를 찾으므로 실행 위치에 영향을 받지 않는다.
        """
    ...
    def _map_to_korean(data: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            korean_key = FIELD_TO_COLUMN.get(key, key)
    ...
    def predict(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Numpy 스칼라를 float로 변환해 JSON 직렬화가 가능하게 한다.

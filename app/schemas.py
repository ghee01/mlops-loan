...
class LoanRequest(BaseModel):
    ...
    ge/le → 모델이 학습한 범위 밖의 입력을 API 단계에서 미리 차단
        ex) 나이 200세, 신용점수 9999점
            → 학습 데이터에 없던 값을 모델에 넣으면 예측 결과를 신뢰할 수 없다.
              Pydantic이 이런 값을 모델 호출 전에 422 에러로 걸러주므로 model.py에는 별도의 방어코드가 필요없다.
    """
    age: int = Field(
        ...,
        ge=19,  # >= 이상 (gt >)
    ...
    repayment_method: str = Field(
        ...,
        description='상환방식',
        examples=['원리금균등']     # 원금균등, 원리금균등, 만기일시
    )
...
class LoanResponse(BaseModel):
    ...
    probability: float = Field(
        ...,
        ge=0.0, # 유효 범위 다시 검증, model.py 내부에서 이미 0~1 값만 나온다.
        le=1.0, # → 응답 스키마에도 제한을 두면 혹시 모를 계산 실수까지 이중으로 방어할 수 있다.
...
class BatchLoanRequest(BaseModel):
    ...
    requests 필드 하나에 LoanRequest 리스트를 통째로 담는다.
        리스트를 감싸는 모델을 만드는 이유
        → 리스트를 바로 요청 본문으로 받으면 (list[LoanRequest]) FastAPI가 최상위 배열을 검증하는 방식이 까다로워지고,
          나중에 페이지네이션 정보 등 다른 필드를 추가하기도 어렵기 때문
...
#   - 기존 LoanResponse는 다른 곳에서 사용되므로 건드리지 않고,
#     /predict 하나에만 적용할 "확장판"을 별도 클래스로 새로 만든다.
# --------------------------------------------------------------------------------------
class EnhancedLoanResponse(BaseModel):
    """
    request_id / timestamp를 추가해 운영 중 특정 예측 요청을 추적할 수 있게 한 응답 스키마
    """

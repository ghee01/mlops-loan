...
# __name__ 기반 로거를 사용한 로그에 현재 모델 이름이 함께 기록된다.
...
async def lifespan(app: FastAPI):
    """
    FastAPI 서버의 시작(startup)과 종료(shutdown) 생명 주기를 관리한다.

    yield 이전은 서버가 요청을 받기 전에 한 번 실행되고, 이후는 서버가 종료될 때 한 번 실행된다.
...
async def health_check():
    ...
    로드밸런서(ALB)가 이 엔드포인트를 주기적으로 호출해서 서버 상태를 확인한다.
...
async def predict(request: LoanRequest):
    ...
    FastAPI는 함수 호출 전에 JSON 요청 본문을 LoanRequest로 검증하고,
    반환값도 EnhancedLoanResponse 형식에 맞는지 다시 확인한다.
    """
    model = app.state.model

    try:
        # Pydantic 객체를 순수한 dict로 바꾸어 모델 계층과 API 계층을 분리한다. (느슨한 결합)
        result = model.predict(request.model_dump())

        # --------- 요청 추적용 메타데이터 (신규 추가) -----------------------------------------------------
        # request_id / timestamp는 "무엇을 예측했는가"가 아니라, "언제, 어떤 요청이었는가"를 남기는 정보이므로
        # model.py가 아니라 main.py API 계층에서 만든다.
        # model.py의 predict()는 그대로 두고, 이 값만 API 응답에 얹는다.
        #   - request_id → 매 요청마다 새로 발급받는 UUID.
        #                  로그에서 "이 요청 하나"를 정확히 찾아낼 수 있는 키가 된다.
        #   - timestamp → UTC 기준 ISO 8601 문자열
        #                 timezone.utc를 명시해야 서버가 어느 시간대에 있든 로그 시각 해석이 항상 같다.

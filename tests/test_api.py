...
실행 방법 - 프로젝트 루트(mlops-loan/)에서 실행
핵심 전략 - 학습된 모델 파일(.pkl)이 없어도 예측 결과를 우리가 원하는 값으로 고정하고 테스트
...
def _make_valid_request(**overrides) -> dict:   # ** → dict 개수 모를 때
    """
...
def _set_probabilities(model: LoanModel, *probs: float) -> None:    # * → 패킹
    """
    Mock 파이프라인이 돌려줄 "승인 확률"을 지정한다.

    predict_proba → [[거절확률, 승인확률], ...] 형태의 2차원 배열 반환
    model.py는 이 중 [:, 1] 승인 확률만 꺼내 쓰므로 같은 모양으로 만들어준다.
    """
...
def mock_model():
    ...
    실제 서버는 lifespan에서 model.load()로 pkl 파일을 읽지만,
    테스트에서는 pkl 파일이 없어도 되도록 진짜 대신 가짜(MagicMock)를 끼워넣는다.
    ...
    # 진짜 파이프라인 대신 가짜 → 기본 예측 결과는 "승인 확률 0.8"로 고정
    ...
    yield model  # 여기서 테스트 실행

    # 테스트 끝나면 app.state를 원래대로 되돌린다. (다음 테스트에 영향 방지)
    del app.state.model
...
def test_get_risk_grade_boundaries(probability, expected_grade):
    """
    @pytest.mark.parametrize = 같은 테스트를 여러 입력값으로 반복 실행
...
def test_map_to_korean_converts_field_names():
    """API의 영문 키가 학습 데이터의 한글 컬럼명으로 바뀌는지 검사"""
...
def test_predict_batch_empty_list_returns_empty(mock_model):
    """빈 리스트를 넣으면 모델을 호출하지 않고 빈 리스트를 반환"""
...
def test_root(client):
    """루트 엔드포인트가 200과 data 키를 돌려주는지 검사"""
    response = client.get('/')
    assert response.status_code == 200
    assert 'data' in response.json()

def test_health_when_model_loaded(client):
    """모델이 적재된 상태 - status는 healthy, model_loaded는 True"""
...
def test_health_when_model_not_loaded(client, mock_model):
    ...
    pipeline을 None으로 바꾸면 서버쪽에도 그대로 반영된다.
    """
...
def test_predict_threshold_boundary(client, mock_model):
    """
    임계값 경계: 확률이 정확이 0.5이면 승인으로 처리 (probability >= threshold)
    """
...
def test_predict_response_has_tracking_metadata(client):
    """
    응답에 요청 추적용 request_id(UUID), timestamp(UTC ISO 8601)가 들어있는지 검사
    이 값들은 매번 달라지므로 "값"이 아니라 "형식"이 올바른지를 검사
    """
    body = client.post('/predict', json=_make_valid_request()).json()

    # UUID 형식이 아니면 uuid.UUID()가 ValueError를 일으켜 테스트 실패
...
def test_predict_passes_features_in_training_order(client, mock_model):
    """
    모델에 전달되는 DataFrame의 컬럼 순서가 feature_names(학습 때 순서)와 같은지 검사
    
    학습과 서빙의 컬럼 순서가 어긋나면 Training-Serving Skew가 생겨서 에러 없이
    ...
    assert df.iloc[0]['나이'] == 42  # 영문 age → 한글 나이로 매핑되어 전달
...
def test_predict_returns_422_on_unseen_category(client, mock_model):
    ...
    assert response.status_code == 422
    assert response.json()["detail"] == "입력값 처리 오류"
